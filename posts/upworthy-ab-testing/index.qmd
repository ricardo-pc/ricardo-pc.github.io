...
date: "2026-05-27"
categories: [Experimentation, NLP, Statistical Computing]
...
All ten statistically significant features retain their sign and significance across all four specifications. The coefficient estimates are stable; only the confidence intervals change. The progression also caught one false positive: a negation-words flag ("not", "never", "no") that looks significant under naive OLS but dissolves once errors are clustered by test. That is exactly the kind of finding that would have shipped if the boring step had been skipped.
...
Sentiment tells the story most people don't expect from Upworthy, the internet's most famously optimistic publisher. The VADER score has a negative coefficient: moving a headline from neutral to fully positive sentiment predicts roughly 12% *fewer* clicks. Negativity wins. The audience of a site built on uplift still clicked the darker framing. The effect is associational rather than causal: sentiment was measured on the headlines editors happened to write, not assigned at random.
